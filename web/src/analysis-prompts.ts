import configured from "../prompts/analysis-v16.json" with { type: "json" };
...
const toolKeys = ["CANDIDATE_TOOL_DESCRIPTION", "INSPECTED_COMPARISON_INTENT_PROMPT", "NAMED_CANDIDATE_TOOL_DESCRIPTION", "DISCOVERY_INTENT_PROMPT", "OPERATIONAL_TOOL_DESCRIPTION"] as const;
export type AnalysisPrompts = Readonly<{ version: string; prompts: Readonly<Record<typeof keys[number], string> & Partial<Record<typeof ivKeys[number] | typeof performanceKeys[number] | typeof toolKeys[number], string>>> }>;
...
export function readAnalysisPrompts(value: unknown): AnalysisPrompts {
  ...
  const selectedKeys = [...keys, ...(ivKeys.some(key => Object.hasOwn(raw, key)) ? ivKeys : []), ...(performanceKeys.some(key => Object.hasOwn(raw, key)) ? performanceKeys : []), ...toolKeys.filter(key => Object.hasOwn(raw, key))];
  ...
  const prompts = {} as Record<typeof keys[number], string> & Partial<Record<typeof ivKeys[number] | typeof performanceKeys[number] | typeof toolKeys[number], string>>;
