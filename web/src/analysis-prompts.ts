import configured from "../prompts/analysis-v12.json" with { type: "json" };
...
export type AnalysisPrompts = Readonly<{ version: string; prompts: Readonly<Record<typeof keys[number], string> & Partial<Record<typeof ivKeys[number], string>>> }>;
...
export function readAnalysisPrompts(value: unknown): AnalysisPrompts {
  ...
  const selectedKeys = ivKeys.some(key => Object.hasOwn(raw, key)) ? [...keys, ...ivKeys] : [...keys];
  ...
  const prompts = {} as Record<typeof keys[number], string> & Partial<Record<typeof ivKeys[number], string>>;
