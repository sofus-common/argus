...
export function capturedProvenance(snapshot: MarketSnapshot) {
  ...
  const spotSourceTimes = { bid: source(snapshot.spotSourceTimes?.bid), ask: source(snapshot.spotSourceTimes?.ask) };
  if (timestamp(snapshot.spotAsOf) !== [spotSourceTimes.bid, spotSourceTimes.ask].sort()[0]) throw new Error("Invalid spot provenance");
...
export function validatedSnapshot(snapshot: MarketSnapshot): MarketSnapshot {
  ...
  if (terms !== undefined && (!terms || typeof terms !== "object" || Array.isArray(terms) || Object.keys(terms).length !== 4 || terms.exerciseStyle !== "American" || terms.settlement !== "physical-shares" || terms.sharesPerContract !== 100 || terms.settlementSession !== "PM")) throw new Error("Invalid contract terms");
...
return { ...(snapshot.imported ? { imported: true as const } : {}), id: snapshot.id, underlying, strikeCenter, ...(terms ? { contractTerms: { ...terms } } : {}), ...(snapshot.historical ? { historical: true as const } : {}), source: snapshot.source, spot: snapshot.spot, retrievedAt: snapshot.retrievedAt, spotAsOf: snapshot.spotAsOf, ...(provenance ? { captureSource: "DXLink", spotSourceTimes: provenance.spotSourceTimes } as const : {}), availableExpiries: [...snapshot.availableExpiries], contracts };
}
