...
export function outlookPreset(label: string, spot: number): { target: number; lowerTarget?: number; families: Domain['families'] } | null {
  const offset = ({ 'Very bearish': -.06, Bearish: -.03, Neutral: 0, 'Either direction': .06, Bullish: .03, 'Very bullish': .06 } as Record<string, number>)[label]
  ...
  if (label === 'Either direction') {
    const lowerTarget = Number((spot * .94).toFixed(3))
    return lowerTarget > 0 && lowerTarget < spot && target > spot ? { target, lowerTarget, families: ['long-straddle', 'long-strangle', 'inverse-iron-butterfly', 'inverse-iron-condor'] } : null
  }
...
export async function checkSearch(raw: unknown, state: StrategyState, snapshot: MarketSnapshot, input: Search, domain: Domain | undefined, signal: AbortSignal): Promise<CandidateSearchResult> {
  ...
  for (const [index, candidate] of result.candidates.entries()) {
    ...
    const downsidePnl = input.objective === 'two-sided-pnl' ? (await requestWorkspaceValuation({ ...next, scenarioSpot: input.lowerTargetSpot! }, signal)).metrics.scenarioPnl : undefined
    if (candidate.downsidePnl !== downsidePnl) throw new Error('Candidate downside P/L could not be reconciled.')
    const score = input.objective === 'two-sided-pnl' ? Math.min(metrics.scenarioPnl, downsidePnl!) : input.objective === 'balanced' ? balancedCandidateScore(metrics.scenarioPnl / risk, probability.probability ?? NaN, input.chanceWeight!) : input.objective === 'expiry-probability' ? probability.probability : input.objective === 'return-on-risk' ? metrics.scenarioPnl / risk : metrics.scenarioPnl
...
export function CandidateSearch({ state, snapshot, disabled, onSearch, onInspect, renderComparison, initialTarget, expanded = false, onLoadQuotes, retainedExpiries }: { state: StrategyState; snapshot?: MarketSnapshot; disabled: boolean; onSearch: () => void; onInspect: (search: CandidateSearchResult, snapshot: MarketSnapshot, id: string) => void; renderComparison: (states: StrategyState[]) => ReactNode; initialTarget?: { targetSpot: number; targetDate: string }; expanded?: boolean; onLoadQuotes?: (dates: string[], center: number) => Promise<unknown>; retainedExpiries?: string[] }) {
  ...
  const [lowerTarget, setLowerTarget] = useState(String(Number(((snapshot?.spot ?? state.spot) * .94).toFixed(3))))
  ...
  const validTargets = objective !== 'two-sided-pnl' || !!lowerTarget.trim() && Number.isFinite(Number(lowerTarget)) && Number(lowerTarget) > 0 && Number(lowerTarget) < (snapshot?.spot ?? state.spot) && Number(target) > (snapshot?.spot ?? state.spot)
  const valid = validTargets && !!target.trim() && Number(target) > 0 && Number(target) <= 1000000 && !!loss.trim() && Number.isFinite(Number(loss)) && Number(loss) > 0 && !!fee.trim() && Number.isFinite(Number(fee)) && Number(fee) >= 0 && validDate && (!expiry || expiries.includes(expiry)) && families.length > 0 && !!outlay.trim() && Number.isFinite(Number(outlay)) && Number(outlay) >= 0 && supported && (!hasMixed || !['expiry-probability', 'balanced'].includes(objective))
  const search = async () => {
    ...
    const input: Search = { targetSpot: Number(target), targetDate: new Date(`${date}Z`).toISOString(), maxLoss: Number(loss), feeAllowance: Number(fee), basis, objective, ...(objective === 'two-sided-pnl' ? { lowerTargetSpot: Number(lowerTarget) } : objective === 'balanced' ? { chanceWeight } : {}) }
  ...
  return <section className={`verified-risk${expanded ? ' compact-optimizer' : ''}`} aria-label="Strategy optimizer"><details open={expanded || undefined}><summary>Find a strategy · deterministic optimizer</summary>
    ...
    <form onSubmit={event => { event.preventDefault(); void search() }}><fieldset className="chain-window" disabled={disabled}>
      {expanded && <div className="opt-outlooks quoted-outlooks"><h2>Market outlook</h2><div>{[['Very bearish', '↓↓'], ['Bearish', '↓'], ['Neutral', '→'], ['Either direction', '↔'], ['Bullish', '↑'], ['Very bullish', '↑↑']].map(([label, icon]) => {
        ...
        return <button type="button" key={label} aria-pressed={!!preset && (label === 'Either direction' ? objective === 'two-sided-pnl' && Number(lowerTarget) === preset.lowerTarget : objective !== 'two-sided-pnl') && Number(target) === preset.target && families.length === preset.families.length && preset.families.every(family => families.includes(family))} disabled={!preset} title="Sets target presets and starter families; not a forecast." onClick={() => { if (!preset) return; invalidate(); setTarget(String(preset.target)); setFamilies(preset.families); if (preset.lowerTarget !== undefined) { setLowerTarget(String(preset.lowerTarget)); setObjective('two-sided-pnl') } else if (objective === 'two-sided-pnl') setObjective('target-pnl') }}><b aria-hidden="true">{icon}</b><span>{label}</span></button>
      ...
      <label>{objective === 'two-sided-pnl' ? 'Up-move target price' : 'Target price'}<input aria-label="Optimizer target price" type="number" min="0.001" max="1000000" step="any" required value={target} onChange={event => { invalidate(); setTarget(event.target.value) }} /></label>
      {objective === 'two-sided-pnl' && <label>Down-move target price<input aria-label="Optimizer down-move target price" type="number" min="0.001" max={snapshot?.spot ?? state.spot} step="any" required value={lowerTarget} onChange={event => { invalidate(); setLowerTarget(event.target.value) }} /></label>}
      ...
      <label>Rank by<select aria-label="Optimizer objective" value={objective} onChange={event => { invalidate(); setObjective(event.target.value as Search['objective']) }}><option value="target-pnl">Target P/L</option><option value="two-sided-pnl">Weaker of two target P/Ls</option><option value="return-on-risk">Target P/L / loss bound</option><option value="expiry-probability" disabled={hasMixed}>Modeled expiry profit probability</option>{expanded && <option value="balanced" disabled={hasMixed}>Return / chance blend</option>}</select></label>
      {objective === 'two-sided-pnl' && <p>Ranks the lower P/L at the down and up targets, with the same date, IV and costs. Two scenarios only—not expected return or worst-case loss across all prices. Inspect uses the up-move target.</p>}
    ...
    {!validTargets && <p role="alert">Choose a down target below the quoted spot and an up target above it.</p>}
    ...
    {result && quotesReady && snapshot && <section className="quoted-results" aria-label="Deterministic quoted candidates"><header><h2>Compare ways to express your view</h2><span>{result.evaluated} checked · {result.eligible} eligible · {result.candidates.length} shown</span></header><details className="quoted-search-details"><summary>Search coverage, assumptions and ranking</summary><p>{result.coverage}</p><p>{result.assumptions}</p>
      ...
      <div className="quoted-candidate-grid">{result.candidates.map((candidate, index) => <article className="quoted-candidate-card" key={candidate.id}>
        ...
        <dl className="quoted-candidate-metrics"><div><dt>{result.request.objective === 'two-sided-pnl' ? 'Up-move P/L' : 'Target P/L'}</dt><dd>{money(candidate.metrics.scenarioPnl)}</dd></div>{candidate.downsidePnl !== undefined && <div><dt>Down-move P/L</dt><dd>{money(candidate.downsidePnl)}</dd></div>}<div><dt>{candidate.lossBound ? 'First-expiry loss bound' : 'Expiry max loss'}</dt><dd>{money(candidate.lossBound?.amount ?? candidate.metrics.maxLoss)}</dd></div><div><dt>Net entry outlay</dt><dd>{money(entryOutlay(candidate.state))}</dd></div><div><dt>Modeled expiry chance</dt><dd>{candidate.probability.probability === null ? 'Unavailable' : `${(candidate.probability.probability * 100).toFixed(1)}%`}</dd></div></dl>
        ...
        {result.request.objective === 'two-sided-pnl' && <p>Down {money(result.request.lowerTargetSpot!)} / up {money(result.request.targetSpot)} at {result.request.targetDate}. Weaker target P/L: {money(candidate.score)}.</p>}
