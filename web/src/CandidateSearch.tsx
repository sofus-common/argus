...
export async function checkSearch(raw: unknown, state: StrategyState, snapshot: MarketSnapshot, input: Search, domain: Domain | undefined, signal: AbortSignal): Promise<CandidateSearchResult> {
  ...
  if (domain !== undefined && (!domain || Object.keys(domain).sort().join() !== 'families,maxEntryOutlay' || !Array.isArray(domain.families) || !domain.families.length || new Set(domain.families).size !== domain.families.length || domain.families.some(family => !Object.hasOwn(familyLabels, family)) || !Number.isFinite(domain.maxEntryOutlay) || domain.maxEntryOutlay < 0 || domain.families.some(mixedFamily) && (state.valuationModel !== 'american-crr-1024-v1' || input.objective === 'expiry-probability'))) throw new Error('Invalid candidate search domain.')
