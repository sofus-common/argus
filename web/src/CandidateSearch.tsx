...
import { CANDIDATE_OPTION_FAMILIES, balancedCandidateScore, candidateOptionFamily, candidateStrategyFamily, expirationProbability, firstExpirySpreadLossBound, validateMarketStrategy, type MarketSnapshot, type StrategyState, type searchCandidates } from './options'
...
export function CandidateSearch({ state, snapshot, disabled, onSearch, onInspect, renderComparison, initialTarget, expanded = false }: { state: StrategyState; snapshot?: MarketSnapshot; disabled: boolean; onSearch: () => void; onInspect: (search: CandidateSearchResult, snapshot: MarketSnapshot, id: string) => void; renderComparison: (states: StrategyState[]) => ReactNode; initialTarget?: { targetSpot: number; targetDate: string }; expanded?: boolean }) {
  ...
  const valid = !!target.trim() && Number(target) > 0 && Number(target) <= 1000000 && !!loss.trim() && Number.isFinite(Number(loss)) && Number(loss) > 0 && !!fee.trim() && Number.isFinite(Number(fee)) && Number(fee) >= 0 && validDate && families.length > 0 && !!outlay.trim() && Number.isFinite(Number(outlay)) && Number(outlay) >= 0 && supported && (!hasMixed || !['expiry-probability', 'balanced'].includes(objective))
  const search = async () => {
  ...
  return <section className="verified-risk" aria-label="Strategy optimizer"><details open={expanded || undefined}><summary>Find a strategy · deterministic optimizer</summary>
    ...
    <form onSubmit={event => { event.preventDefault(); void search() }}><fieldset className="chain-window" disabled={disabled}>
      ...
      })}</div><p>Target presets: −6%, −3%, unchanged, +3%, +6% from {quotesReady ? 'dated spot' : 'the displayed workspace spot (not a market quote)'}—not forecasts. Starter option families only; customize below to include other structures or stock. Either-direction scoring is not implemented. Your saved thesis and position are unchanged.</p></div>}
    ...
    {result && quotesReady && snapshot && <section aria-label="Deterministic quoted candidates"><p>{result.coverage}</p><p>{result.assumptions}</p>
