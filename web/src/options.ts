...
export function searchCandidates(context: StrategyState, snapshot: MarketSnapshot, input: { targetSpot: number; targetDate: string; maxLoss: number; feeAllowance: number; basis: PricingBasis; objective: "target-pnl" | "return-on-risk" | "expiry-probability" | "balanced" | "two-sided-pnl"; chanceWeight?: number; lowerTargetSpot?: number }, domain?: CandidateSearchDomain) {
  ...
  const downsidePrices = input.objective === 'two-sided-pnl' ? new Map([...longLegs].map(([id, leg]) => [id, priceLeg(base([leg]), leg, input.lowerTargetSpot!, Date.parse(input.targetDate), true).price])) : undefined;
  ...
  const ranked: Array<{ id: string; state: StrategyState; score: number; pnl: number; downsidePnl?: number; lossBound?: ReturnType<typeof firstExpirySpreadLossBound> }> = [];
  ...
  const evaluate = (legs: OptionLeg[], stock = false, mixedExpiry = false) => {
    ...
    const downsidePnl = downsidePrices ? rounded(legs.reduce((sum, leg) => sum + downsidePrices.get(leg.contractId)! * (leg.side === 'long' ? 1 : -1) * leg.contracts * leg.multiplier, stock ? 100 * input.lowerTargetSpot! : 0) - entryCost(state) - input.feeAllowance) : undefined;
    const score = input.objective === 'two-sided-pnl' ? Math.min(pnl, downsidePnl!) : input.objective === 'balanced' ? balancedCandidateScore(pnl / loss, probabilityFor(state).probability!, input.chanceWeight!) : input.objective === "expiry-probability" ? probabilityFor(state).probability : input.objective === "target-pnl" ? pnl : pnl / loss;
    if (!finite(pnl) || !finite(score) || downsidePrices && !finite(downsidePnl)) throw new Error("Candidate numerical range exceeded");
    eligible++;
    const candidate = { id: (stock ? `stock:100@${snapshot.spot}|` : '') + legs.map(leg => `${leg.side}:${leg.contractId}${leg.contracts === 1 ? "" : `*${leg.contracts}`}`).join("|"), state, score, pnl, ...(downsidePrices ? { downsidePnl } : {}), ...(lossBound ? { lossBound } : {}) };
  ...
  const result = {
    ...
    candidates: ranked.map(({ id, state, score, downsidePnl, lossBound }) => ({ id, state, score, metrics: calculateStrategy(state), probability: probabilityFor(state), ...(downsidePnl !== undefined ? { downsidePnl } : {}), ...(lossBound ? { lossBound } : {}) })),
  };
  if (input.objective === 'two-sided-pnl') result.assumptions += ' Ranking uses the weaker of two finite conditional P/L scenarios at the supplied lower and upper target prices, with the same date, IV, quote basis and fee allowance. It is not expected P/L or a global worst-case loss; losses between or beyond the targets may be worse. Candidate metrics and Inspect use the upper target; downsidePnl is the lower target outcome.';
  return result;
}
