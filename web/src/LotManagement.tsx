...
function LotScenarios({ comparison, savedId, transaction }: { comparison: Comparison; savedId: string; transaction: LotTransaction }) {
  ...
  return <section aria-label="Roll price and time scenarios"><h3>Price and time scenarios</h3>
    ...
    <form onSubmit={event => { event.preventDefault(); void model() }}><fieldset><legend>Shared assumptions</legend>
      <label>Underlying price<input aria-label="Comparison scenario spot" type="number" min="0.000001" step="any" required value={spot} onChange={event => setSpot(event.target.value)} /></label>
    ...
    {value && <section ref={chart} tabIndex={-1} aria-label="Modeled roll comparison">{preview && <p>Conversation chart preview · selected controls and saved position are unchanged. <button onClick={() => setPlotted(null)}>Return to selected scenario</button></p>}<p>{value.model} · {value.scenario.date} · IV shift {(value.scenario.ivShift * 100).toFixed(2)} pp</p>
      <dl className="lifecycle-totals"><div><dt>Held at {money(value.scenario.spot)}</dt><dd>{money(value.before.target)}</dd></div><div><dt>Proposed at {money(value.scenario.spot)}</dt><dd>{money(value.after.target)}</dd></div><div><dt>Proposed difference</dt><dd>{money(value.after.target - value.before.target)}</dd></div></dl>
      <svg viewBox="0 0 620 250" role="img" aria-label="Held and proposed modeled P/L curves" style={{ width: '100%', display: 'block' }} onPointerMove={event => {
        ...
        <g fill="var(--muted)" fontSize="11"><text x="70" y="17" textAnchor="end">{money(high)}</text><text x="70" y="214" textAnchor="end">{money(low)}</text><text x="74" y="238">{money(value.min)}</text><text x="584" y="238" textAnchor="end">{money(value.max)}</text></g>
      </svg>{inspected && <><label>Inspect comparison price<input type="range" aria-label="Inspect comparison price" min="0" max={samples.length - 1} step="1" value={inspectedIndex} aria-valuetext={`${money(inspected.spot)}; held ${money(inspected.held)}; proposed ${money(inspected.proposed)}`} onChange={event => setInspection({ source: value, index: Number(event.target.value) })} /></label><output role="status" aria-label="Comparison price inspection" aria-live="polite">Sampled price {money(inspected.spot)} · held {money(inspected.held)} · proposed {money(inspected.proposed)} · difference {money(inspected.proposed - inspected.held)}</output></>}<p>Dashed blue: held. Solid green: proposed. Hover or use the price slider and arrow keys to inspect shared calculated samples. Combined P/L includes the allowance once per side. Curves show this scenario date, not guaranteed returns.</p>
    </section>}
...
export function LotManagement({ savedId, source, snapshotId, onClose, onRecorded, onAnalyze }: { savedId: string; source: StrategyState; snapshotId?: string; onClose: () => void; onRecorded: () => Promise<void>; onAnalyze: (state: StrategyState, snapshotId: string) => boolean }) {
  ...
  const candidates: { id: string; asset: LotAsset; side: 'long' | 'short'; quantity: number }[] = [
    ...
    ...(captured.stock ? [{ id: 'stock', asset: { kind: 'stock' as const, symbol: captured.underlying }, side: captured.stock.shares > 0 ? 'long' as const : 'short' as const, quantity: Math.abs(captured.stock.shares) }] : []),
  ]
  const sameUnderlying = current?.record.state.underlying === captured.underlying
  ...
  const inventory = (projection: Projection) => projection.lots.length ? <ul className="lot-inventory">{projection.lots.map(lot => <li key={lot.id}><strong>{lot.side} {label(lot.asset)}</strong><br />Lot {lot.id} · {lot.quantity} {lot.asset.kind === 'stock' ? 'shares' : 'contracts'} · held entry ${lot.entryPrice} per share<br /><small>Opened {lot.at}</small></li>)}</ul> : <p>No remaining lots. Position fully closed.</p>
  const compare = async () => {
  ...
  return <dialog ref={dialog} className="lifecycle-dialog lot-management" aria-label="Manage lots and rolls" onCancel={event => { event.preventDefault(); if (!busy && !uncertain) dismiss() }}>
    ...
    {current && <><h3>Remaining lot inventory</h3>{inventory(current.projection)}{totals(current.projection)}
      ...
      {chosenAction !== 'transaction' && !reviewing && <form onSubmit={event => { event.preventDefault(); void previewAmendment() }}><fieldset disabled={busy || uncertain}><legend>{chosenAction === 'opening-correction' ? 'Correct a recorded opening price' : chosenAction === 'correction' ? 'Correct a recorded close price' : 'Void an erroneous close'}</legend><p>{chosenAction === 'void' ? 'Only void a close that did not occur. This restores recorded inventory; it does not reverse an actual trade or send an order.' : chosenAction === 'opening-correction' ? 'Correct the opening transcription price only. This recalculates the lot’s realized P/L and remaining entry basis, including fully closed lots. Quantity, contract, execution time and original records are unchanged. This does not void an opening or undo a roll.' : 'Correct the transcription price only. Quantity and execution time are unchanged; the original record is retained.'}</p>
        {chosenAction === 'opening-correction' ? <label>Recorded opening lot<select aria-label="Recorded opening lot" required value={closeId} onChange={event => setCloseId(event.target.value)}><option value="">Select an opening lot</option>{current.projection.openings.map(lot => <option key={lot.id} value={lot.id}>{lot.id} · {lot.side} {label(lot.asset)} · opened {lot.quantity} · original ${lot.originalEntryPrice} · effective ${lot.entryPrice}{current.projection.lots.some(item => item.id === lot.id) ? '' : ' · fully closed'}</option>)}</select></label> : <label>Recorded close<select aria-label="Recorded lot close" required value={closeId} onChange={event => setCloseId(event.target.value)}><option value="">Select a close</option>{activeCloses.map(item => <option key={item.id} value={item.id}>{item.description} · original ${item.price} · effective ${effectivePrice(item.id, item.price)}</option>)}</select></label>}
        {chosenAction !== 'void' && <label>Corrected price per share<input aria-label={chosenAction === 'opening-correction' ? 'Corrected lot opening price' : 'Corrected lot close price'} type="number" min="0" step="any" required value={correctedPrice} onChange={event => setCorrectedPrice(event.target.value)} /></label>}<label>Reason<input aria-label="Lot amendment reason" required maxLength={500} value={reason} onChange={event => setReason(event.target.value)} /></label><button type="submit">Preview lot amendment</button></fieldset></form>}
      {amendmentReview && <section aria-label="Lot amendment preview"><h3>Preview amendment recording</h3><p>{amendmentReview.amendment.kind === 'opening-price-correction' ? `Opening lot ${amendmentReview.amendment.lotId}` : `Close ${amendmentReview.amendment.closeId}`}{amendmentReview.amendment.kind !== 'close-void' ? ` · corrected price $${amendmentReview.amendment.price} per share` : ' · void erroneous close; not a reversal of an actual trade or an order'}<br />{amendmentReview.amendment.reason}<br />Requested recording timestamp {amendmentReview.amendment.recordedAt}</p>
        {amendmentReview.amendment.kind === 'opening-price-correction' && <p>Opening basis per share: ${amendmentReview.before.openings.find(lot => amendmentReview.amendment.kind === 'opening-price-correction' && lot.id === amendmentReview.amendment.lotId)?.entryPrice} → ${amendmentReview.amendment.price}. Applies to closed and remaining units of this lot; no quantity, identity or execution-time change.</p>}
      ...
      {!review && <form onSubmit={event => { event.preventDefault(); void preview() }}><fieldset disabled={busy || uncertain} className="lot-transaction"><legend>Record a transaction</legend><p>Close specific held lots and optionally open new lots together. Execution prices must be entered; they are not quotes.</p><div><h3>Close held lots</h3>{current.projection.lots.map(lot => controls('Close', lot.id, lot.quantity, `${lot.side} ${label(lot.asset)}`))}</div><div><h3>Open from captured workspace</h3><p>{captured.name} · {captured.underlying} · version {captured.version}. These are descriptive source legs, not current quotes or recorded fills.</p>{sameUnderlying ? candidates.map(item => controls('Open', item.id, item.quantity, `${item.side} ${label(item.asset)}`)) : <p>The captured workspace has a different underlying. Only closing held lots is available.</p>}</div><label>Transaction UTC datetime<input type="datetime-local" step="1" aria-label="Transaction UTC datetime" required value={at} onChange={event => setAt(event.target.value)} /></label><button type="submit">Preview transaction</button></fieldset></form>}
      {review && <section aria-label="Transaction preview"><h3>Preview transaction recording</h3><p>Execution {review.transaction.at}<br />Requested recording timestamp {review.transaction.recordedAt}</p><ul>{review.transaction.closes.map(close => <li key={close.id}>Close lot {close.lotId}: {close.quantity} at ${close.price} per share</li>)}{review.transaction.opens.map(open => <li key={open.id}>Open {open.side} {label(open.asset)}: {open.quantity} at ${open.entryPrice} per share</li>)}</ul><h3>Inventory before</h3>{inventory(review.before)}<h3>Projected inventory after confirmation</h3>{inventory(review.projection)}<p>Projected gross realized change {money(review.projection.grossRealizedPnl - review.before.grossRealizedPnl)}. This previews execution accounting after confirmation, not a quote or modeled price scenario.</p><p>{uncertain ? 'Recording status is uncertain; this transaction may already be saved. Retry the identical request or reload to confirm.' : 'These are projected recorded totals after confirmation, not confirmation that the transaction is saved.'}</p>{totals(review.projection, true)}<button disabled={busy} onClick={() => void confirm()}>{uncertain ? 'Retry identical transaction' : 'Confirm recorded transaction'}</button><button disabled={busy || uncertain} onClick={() => { setReview(null); setError('') }}>Edit transaction</button></section>}
      </div>
<details><summary>Recorded lot activity</summary><h3>Effective recorded closes</h3><ul>{recordedCloses.map(item => <li key={item.id}>{item.id} · {item.description} · original ${item.price} · effective ${effectivePrice(item.id, item.price)}{voided.has(item.id) ? ' · Voided; excluded from realized P/L' : ''}</li>)}</ul><h3>Original held position</h3><p>{current.record.state.name} · {current.record.state.valuationTimestamp}</p><ul>{current.record.state.legs.map(leg => <li key={leg.id}>{leg.id} · {leg.side} {leg.contractId} · {leg.contracts} contracts at ${leg.entryPrice}</li>)}{current.record.state.stock && <li>{current.record.state.stock.shares} shares at ${current.record.state.stock.entryPrice}</li>}</ul><h3>Original close history</h3><ul>{legacy?.closes.map(close => <li key={close.id}>{close.id} · {close.assetId} · {close.quantity} at ${close.price} · {close.at}</li>)}{legacy?.priceCorrections?.map(event => <li key={event.id}>Price correction {event.closeId} to ${event.price} · {event.reason} · {event.recordedAt}</li>)}{legacy?.closeVoids?.map(event => <li key={event.id}>Voided close {event.closeId} · {event.reason} · {event.recordedAt}</li>)}</ul>{ledger?.schemaVersion === 2 && <><h3>Dated transactions</h3>{ledger.transactions.map(transaction => <div key={transaction.id}><p>{transaction.id} · execution {transaction.at} · recorded {transaction.recordedAt}</p><ul>{transaction.closes.map(close => <li key={close.id}>Close {close.id} · lot {close.lotId} · {close.quantity} at ${close.price}</li>)}{transaction.opens.map(open => <li key={open.id}>Open lot {open.id} · {open.side} {label(open.asset)} · {open.quantity} at ${open.entryPrice}</li>)}</ul></div>)}<h3>Appended amendments</h3><ul>{ledger.amendments?.map(event => <li key={event.id}>{event.kind} · {event.kind === 'opening-price-correction' ? `opening lot ${event.lotId}` : `close ${event.closeId}`}{event.kind !== 'close-void' && ` · ${event.price}`} · {event.reason} · {event.recordedAt}</li>)}</ul></>}</details>
    </>}
  </dialog>
}
