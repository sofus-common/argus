...
export function ExpiryStrip({ expiry, onChange, dates = LAB_SAMPLE_EXPIRIES, quoted = false, minDate }: { expiry: string; onChange: (date: string) => void; dates?: readonly string[]; quoted?: boolean; minDate?: string }) {
  ...
  const measure = () => { const node = track.current; if (node) setEdges({ start: node.scrollLeft <= 1, end: node.scrollLeft + node.clientWidth >= node.scrollWidth - 1 }) }
  useEffect(() => {
    ...
    <div className="opt-strip-picker">
      ...
      <div ref={track} className="opt-strip-track" role="group" aria-label="Expiration dates" onScroll={measure}
        ...
        onPointerMove={event => {
          ...
          if (Math.abs(delta) > 5) { start.moved = true; event.currentTarget.setPointerCapture(event.pointerId) }
          if (start.moved) { event.preventDefault(); event.currentTarget.scrollLeft = start.scroll - delta }
        }}
...
export function ExpiryPlot({ state, breakevens = [], referenceSpot = 100 }: { state: StrategyState; breakevens?: number[]; referenceSpot?: number }) {
  ...
  const area = `${path} L${x(max)},${y(0)} L${x(min)},${y(0)} Z`
  return <svg className="opt-payoff" viewBox="0 0 420 248" role="img" aria-label={`Expiration payoff for ${legsLabel(state)}; prices ${min} to ${max}; no probability forecast`}>
    <defs><clipPath id={`${id}-gain`}><rect x="54" y="0" width="342" height={y(0)} /></clipPath><clipPath id={`${id}-loss`}><rect x="54" y={y(0)} width="342" height={248 - y(0)} /></clipPath><linearGradient id={`${id}-green`} x1="0" y1="0" x2="0" y2="1"><stop stopColor="#64e5bb" stopOpacity=".32"/><stop offset="1" stopColor="#64e5bb" stopOpacity=".04"/></linearGradient><linearGradient id={`${id}-red`} x1="0" y1="0" x2="0" y2="1"><stop stopColor="#fa6d82" stopOpacity=".05"/><stop offset="1" stopColor="#fa6d82" stopOpacity=".3"/></linearGradient></defs>
  ...
  return <main className="opt-screen">
    ...
    <section className="opt-results" aria-label="Optimizer results"><div className="opt-results-heading"><div><h1 ref={heading} tabIndex={-1}>Compare ways to express your view</h1><p>Same target, horizon and pricing assumptions. Preview before applying.</p></div><span>{result ? `${result.searched} checked · ${result.eligible} eligible` : 'Synthetic model search · not trading advice'}</span></div>
      ...
      {result && <><p className="opt-results-note">Best eligible result per family, ranked by {objective === 'profit' ? 'target-date dollar profit' : 'target-date P/L divided by expiry maximum loss'}. Same base quantity, not equal exposure. Missing families are outside the search criteria or budgets; results can lose money.</p><div className="opt-cards">{result.candidates.map((item, index) => <article className="opt-card" key={item.family}>
        <header><h2>{item.family}</h2><small>#{index + 1} · {item.state.legs[0].contracts} base quantity</small></header>
        <p className="opt-card-legs">{legsLabel(item.state)}</p>
        <dl className="opt-card-metrics">
          <div><dt>Target return / risk</dt><dd className={item.pnl < 0 ? 'opt-negative' : 'opt-positive'}>{item.returnOnRisk === null ? '—' : `${(item.returnOnRisk * 100).toFixed(1)}%`}</dd></div>
          <div><dt>Target-date P/L</dt><dd className={item.pnl < 0 ? 'opt-negative' : 'opt-positive'}>{money(item.pnl)}</dd></div>
          <div><dt>Expiry max loss</dt><dd className="opt-negative">{money(item.maxLoss)}</dd></div>
        </dl>
        <ExpiryPlot state={item.state} breakevens={item.breakevens}/>
        <dl className="opt-card-secondary"><div><dt>{item.debit >= 0 ? 'Entry outlay' : 'Entry credit'}</dt><dd>{money(Math.abs(item.debit))}</dd></div><div><dt>Expiry max profit</dt><dd>{item.maxProfit === null ? 'Unlimited' : money(item.maxProfit)}</dd></div><div><dt>Breakeven at expiry</dt><dd>{item.breakevens.length ? item.breakevens.map(value => money(value)).join(' / ') : 'None'}</dd></div></dl>
        <details className="opt-card-risk"><summary>{item.collateral > 0 ? `Stock / cash reserved ${money(item.collateral)}` : 'Limited expiry loss · risk details'}</summary><p>{item.riskNote} Probability is not calculated.</p></details>
        <button className="opt-preview-button" onClick={() => setPreview(index)} aria-label={`Preview candidate ${index + 1}`}>Preview candidate</button>
      </article>)}</div>{!result.candidates.length && <p className="opt-status" role="status">No supported strategy meets these loss and stock/cash limits.</p>}</>}
    </section>
