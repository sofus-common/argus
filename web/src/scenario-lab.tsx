...
function ScenarioLab() {
  ...
  const scenarios = useMemo(() => labScenarios(position, position.scenarioSpot, position.scenarioDate), [position])
  ...
  return <main className="lab">
    ...
    <div className="lab-workspace"><div className="lab-main">
      ...
      <LabAnalysisChart state={activeState} onChange={inspect} onPositionChange={next => update({ ...position, legs: next.legs })} onAsk={() => { setQuestion('Explain this chart and challenge the trade assumptions.'); setDiscussion(true) }}/>
      <p className="lab-caption">Chart price, date and IV inspect a scenario without changing your thesis. Dragging strikes edits the hypothetical position; entry premiums stay fixed.</p>
      <section className="lab-analysis" aria-label="Selected scenario analysis"><div className="lab-selected-summary"><strong>{activeName} <b className={greeks.pnl < 0 ? 'lab-negative' : 'lab-positive'}>{dollars(greeks.pnl)}</b></strong><span>At {dollars(activeState.scenarioSpot)} · {shortDate(activeState.scenarioDate)} · IV {(22 + activeState.ivShift * 100).toFixed(0)}%</span></div><dl className="lab-greeks">{([['Delta', greeks.delta, 'USD / $1 move'], ['Gamma', greeks.gamma, 'delta / $1 move'], ['Theta', greeks.theta, 'USD / day'], ['Vega', greeks.vega, 'USD / IV point'], ['Rho', greeks.rho, 'USD / rate point']] as const).map(([name, value, unit]) => <div key={name}><dt>{name}</dt><dd>{value.toFixed(2)}</dd><small>{unit}</small></div>)}</dl></section>
