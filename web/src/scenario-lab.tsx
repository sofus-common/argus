...
function ScenarioLab() {
  ...
  return <main className="lab">
    ...
    <section className="lab-thesis" aria-label="Your trading thesis"><div className="lab-thesis-title"><span className="lab-label">START WITH YOUR VIEW</span></div><div className="lab-thesis-fields"><label className="lab-thesis-copy">My thesis<input id="thesis" value={thesis} maxLength={12000} placeholder="What do you expect, and why?" onChange={e => { revision.current++; setThesis(e.target.value); setNotice('Unsaved changes'); setDiscussion(false) }} /></label><label>Target price $<input aria-label="Target price" type="number" min={snapshot ? .001 : 80} max={snapshot ? 1000000 : 120} step="0.5" value={position.scenarioSpot} onChange={e => update({ ...position, scenarioSpot: e.target.valueAsNumber }, 'thesis')} /></label><label>Thesis horizon<input aria-label="Thesis horizon" type="date" min={position.valuationTimestamp.slice(0, 10)} value={horizon} onChange={e => { revision.current++; setHorizon(e.target.value); setNotice('Unsaved changes'); setDiscussion(false) }} /></label></div></section>
