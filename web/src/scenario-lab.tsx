...
import { calculateStrategy, evaluateScenario, payoffSeries, sampleContractId, LAB_SAMPLE_EXPIRIES, type StrategyState } from './options'
...
function ScenarioLab() {
  ...
  return <main className="lab">
    ...
    <section className="lab-expiry"><label className="lab-label">EXPIRATION <select aria-label="Workbench expiry" value={expiry} onChange={e => { const date = e.target.value; update({ ...position, scenarioDate: position.scenarioDate > date ? date : position.scenarioDate, legs: position.legs.map(leg => ({ ...leg, expiry: date, contractId: sampleContractId(leg.type, leg.strike, date) })) }) }}>{LAB_SAMPLE_EXPIRIES.map(date => <option key={date} value={date}>{shortDate(date)} {date.slice(0, 4)}</option>)}</select></label><p>Synthetic dates · premiums held when editing · no live quotes</p></section>
