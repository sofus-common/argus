import { validateMarketStrategy, validateStrategy, type MarketSnapshot, type StrategyState } from './options';
...
export function readWorkspaceDraft(raw: string): WorkspaceDraft {
  ...
  if (validateStrategy(value.state).length) throw new Error('Invalid draft strategy');
  ...
  if (input.pricing) {
    ...
    if (validateMarketStrategy(state, snapshot).length) throw new Error('Draft pricing does not match');
