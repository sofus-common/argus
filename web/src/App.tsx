      ...
      <div className="workspace">
        ...
        <section className="builder">
          ...
          {marketSnapshot && <div className="market-pricing-panel">
            ...
            <details><summary>Choose chain window</summary><div className="chain-window"><label>Strike center<input aria-label="Strike center" type="number" min="0.001" max="1000000" step="any" value={strikeCenter} onChange={e => setStrikeCenter(e.target.value)} /></label><button disabled={chainPending || !Number.isFinite(Number(strikeCenter)) || Number(strikeCenter) <= 0 || Number(strikeCenter) > 1000000} onClick={() => void loadMarket(true, [...new Set(marketSnapshot.contracts.map(c => c.expiry.slice(0,10)))].sort(), strategy.underlying, Number(strikeCenter))}>Browse strikes</button><small>Retains your selected legs and quantities. {strategy.pricing?.entryMode === 'fixed' ? 'Refreshes quotes; held entry costs remain unchanged.' : 'Refreshes quotes and re-estimates entry.'} Not realized returns.</small></div><div className="chain-window">{[0,1].map(index => <label key={index}>Expiry {index+1}{index === 1 ? ' (optional)' : ''}<select aria-label={`Chain expiry ${index+1}`} value={windowDates[index] ?? ''} onChange={e => setWindowDates(dates => Object.assign([...dates], { [index]: e.target.value }))}><option value="" disabled={index === 0}>{index === 0 ? 'Choose expiry' : 'None — single expiry'}</option>{marketSnapshot.availableExpiries.map(date => <option key={date} value={date}>{date}</option>)}</select></label>)}<button disabled={chainPending || !windowDates[0] || windowDates[0] === windowDates[1]} onClick={() => void loadMarket(false, windowDates.filter(Boolean))}>Rebuild template with these expiries</button><small>Calendars and diagonals need two expiries. Rebuilds the current template; custom legs and quantities are replaced. Refresh prices above preserves your legs.</small></div></details>
          </div>}
              ...
              {message.analysis && <div className="analysis-evidence">
                ...
                {message.analysis.calculated?.candidateSearch && <section className="verified-risk" aria-label="Ranked quoted candidates">
                  ...
                  {message.analysis.calculated.candidateSearch.candidates.map((candidate, rank) => <div key={candidate.id}>
                    ...
                    <button className="preview-chart-button" disabled={pending || chainPending || workspaceBusy || analysisUnavailable || message.analysis!.positionVersion !== strategy.version || message.analysis!.calculated.candidateSearch!.snapshotId !== strategy.pricing?.snapshotId} onClick={() => void inspectCandidate(message.analysis!, candidate.id)}>Inspect candidate</button>
                  </div>)}
