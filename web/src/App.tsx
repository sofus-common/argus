...
export function App({ presentation = 'default' }: { presentation?: 'default' | 'workbench' } = {}) {
  ...
  return (
    <main className={`app-shell${compact ? ` integrated-workbench${optimizerScreen ? ' optimizer-screen' : ''}` : ''}`}>
      ...
      {compact && <nav className="workbench-screen-nav" aria-label="Workbench and optimizer"><button aria-current={!optimizerScreen ? 'page' : undefined} onClick={() => setOptimizerScreen(false)}>{optimizerScreen ? 'Back to Workbench' : 'Workbench'}</button><button aria-current={optimizerScreen ? 'page' : undefined} onClick={() => { stopAutomatic(); setOptimizerScreen(true) }}>Optimize</button><span>{optimizerScreen ? 'Compare alternatives · Inspect returns to Workbench · Apply is separate' : 'Your position · thesis, analysis and conversation'}</span></nav>}
