...
function ScenarioTable({ state, data, error, onSelect, pnlDisplay = 'pnl', selectionNote = 'Select a price to apply that scenario; Undo restores it.' }: { selectionNote?: string; pnlDisplay?: PnlDisplayMode; state: StrategyState; data?: Awaited<ReturnType<typeof requestWorkspaceValuation>>['table']; error?: string; onSelect: (patch: Partial<StrategyState>) => void }) {
...
export { PayoffChart, Heatmap, ScenarioTable, ScenarioInput, ChartRangeControls, chartMetrics }
export type { ChartMetric }

function RequestedScenarios({ scenarios, baseline, onPreview }: { scenarios: SparringSuccess['calculated']['requestedScenarios']; baseline: StrategyState; onPreview?: (result: SparringSuccess['calculated']['requestedScenarios'][number]) => void }) {
  return <><details className="source-evidence requested-scenarios"><summary>Requested what-if calculations</summary><p>Read-only model evaluations of the reviewed position. Entry costs and legs are unchanged; no scenario has been applied.</p>
    {scenarios.map((result, index) => <section key={result.id} aria-label={`Requested scenario ${index + 1}`}><b>Scenario {index + 1}</b>
      <button className="preview-chart-button" disabled={!onPreview} onClick={() => onPreview?.(result)}>Preview chart</button>
...
function ScenarioPreview({ baseline, snapshot, result, onClose }: { baseline: StrategyState; snapshot?: MarketSnapshot; result: SparringSuccess['calculated']['requestedScenarios'][number]; onClose: () => void }) {
  ...
  return <section className="scenario-preview" aria-label="Scenario chart preview" ref={panel} tabIndex={-1} onKeyDown={event => { if (event.key === 'Escape') { event.stopPropagation(); onClose() } }}>
    <header><div><small>READ-ONLY · CAPTURED WHAT-IF</small><h2>Scenario chart preview</h2></div><button onClick={onClose}>Close preview</button></header>
    ...
    {preview.state && checked?.metrics ? <>
      ...
      <p className="preview-context">Solid curve: requested what-if. Comparison: reviewed scenario at {baseline.scenarioDate}, spot ${baseline.scenarioSpot.toFixed(2)}, IV shift {signed(baseline.ivShift * 100, 2)} pts. Dates and IV assumptions may differ; holdings and entry costs do not. {baseline.pricing ? `${baseline.pricing.basis} quote basis · ${baseline.pricing.entryMode} entries.` : 'Sample/manual entry basis.'}</p>
      <PayoffChart state={preview.state} comparison={baseline} comparisonLabel="Reviewed scenario" metric={metric} readOnly />
...
function PositionComparison({ result, baseline: captured, current: capturedMetrics, snapshot }: { result: NonNullable<SparringSuccess['calculated']['positionComparison']>; baseline: StrategyState; current: SparringSuccess['metrics']; snapshot?: MarketSnapshot }) {
  ...
  return <details className="source-evidence requested-scenarios holdings-comparison" aria-label="Read-only holdings comparison"><summary>{result.candidate ? 'Held versus selected quoted candidate · read-only' : 'Calculated holdings comparison · no proposal'}</summary>
    <p>{baseline.underlying} · captured workspace v{baseline.version} · {baseline.scenarioDate} · spot {baseline.underlyingKind === 'cash-index' ? `${baseline.scenarioSpot.toFixed(2)} index points` : `$${baseline.scenarioSpot.toFixed(2)}`} · total global IV shift {signed(baseline.ivShift * 100, 2)} pts</p>
    {(baseline.scenarioSpot !== captured.scenarioSpot || baseline.scenarioDate !== captured.scenarioDate || baseline.ivShift !== captured.ivShift) && <p>Both positions are modeled at this requested scenario. The workspace remains at {captured.underlyingKind === 'cash-index' ? `${captured.scenarioSpot.toFixed(2)} index points` : `$${captured.scenarioSpot.toFixed(2)}`} on {captured.scenarioDate}, global IV shift {signed(captured.ivShift * 100, 2)} pts; no scenario controls were changed.</p>}
    {snapshot ? <details aria-label="Comparison quote timestamps"><summary>Captured quote timestamps · not live fills</summary><p>{snapshot.source} · retrieved {snapshot.retrievedAt}<br />Underlying mark: {snapshot.spotAsOf}</p>{snapshot.contracts.filter(contract => [...baseline.legs, ...result.state.legs].some(leg => leg.contractId === contract.contractId)).map(contract => <p key={contract.contractId}>{contract.contractId} · {contract.quoteAsOf}</p>)}<p>Separate timestamps may be asynchronous. {!result.candidate && 'Additional share purchase price is a hypothetical assumption, not a quoted offer.'} This comparison does not refresh quotes.</p></details> : <p>Sample/manual contracts and prices, not executable market quotes.</p>}
    <p>{result.assumptions}</p>
    ...
    {result.candidate ? <><p>New quoted position on {result.candidate.request.basis} entry basis, not a roll, executed trade or change to held entry costs. Closing proceeds and realized P/L are excluded.</p>{result.candidate.lossBound && <p>Conservative first-expiry loss bound: {money(result.candidate.lossBound.amount)} at {result.candidate.lossBound.date}. {result.candidate.lossBound.basis}</p>}</> : <><p>Additional share purchase assumption: {money(result.additionalShareCost)}. Compared holding: {result.state.stock ? holdingDescription(result.state.stock, baseline.underlying) : 'No shares'}.</p>{!!result.removedLegIds.length && <p>Excluded option legs: {result.removedLegIds.join(' · ')}. Exit proceeds and realized P/L are not included.</p>}</>}
    <div className="leg-risk leg-risk-scroll" role="region" aria-label="Scrollable holdings comparison" tabIndex={0}><table aria-label="Holdings comparison metrics"><thead><tr><th scope="col">Metric</th><th scope="col">Captured holdings</th><th scope="col">Compared holdings</th></tr></thead><tbody>
      <tr><th scope="row">Payoff reference horizon</th>{[baseline, result.state].map((state, index) => {
        ...
        return <td key={index}>{expiry ? <><time dateTime={expiry}>{expiry.slice(0, 10)}<br />{expiry.slice(11, 19)} UTC</time><small>{[current, result.metrics][index].mode === 'first-expiry' ? 'First expiry · sampled only' : 'Intact expiry'}</small></> : 'Stock only · no option expiry'}</td>
      })}</tr>
      ...
      <tr><th scope="row">Intact expiry breakevens · {baseline.underlyingKind === 'cash-index' ? 'index points' : 'USD'}</th>{[current, result.metrics].map((metrics, index) => <td key={index}>{metrics.mode === 'first-expiry' ? 'Not exact' : metrics.breakevens.map(value => baseline.underlyingKind === 'cash-index' ? value.toFixed(3) : money(value, '—', 3)).join(' · ') || 'None'}</td>)}</tr>
      {(['scenarioPnl', 'maxProfit', 'maxLoss', 'delta', 'gamma', 'theta', 'vega', 'rho'] as const).map(key => <tr key={key}><th scope="row">{key === 'scenarioPnl' ? 'Modeled P/L · USD' : key === 'maxProfit' ? 'Intact expiry max profit' : key === 'maxLoss' ? 'Intact expiry max loss' : `${chartMetrics[key].label} · ${baseline.underlyingKind === 'cash-index' ? chartMetrics[key].unit.replace('$1 spot move', '1 index-point move') : chartMetrics[key].unit}`}</th>{[current, result.metrics].map((metrics, index) => <td key={index}>{key === 'maxProfit' || key === 'maxLoss' ? bound(metrics, key) : signed(metrics[key], key === 'gamma' ? 3 : 2)}</td>)}</tr>)}
    </tbody></table></div>
...
export function App({ presentation = 'default' }: { presentation?: 'default' | 'workbench' } = {}) {
  ...
  const cashIndex = strategy.underlyingKind === 'cash-index'
  const showAmericanPreview = americanPreview && !cashIndex
  const americanPreviewActive = useRef(false)
  americanPreviewActive.current = view === 'heatmap' && showAmericanPreview && strategy.valuationModel !== 'american-crr-1024-v1'
  ...
  const [scenarioSelection, setScenarioSelection] = useState<{ analysis: NonNullable<ChatMessage['analysis']>; result: SparringSuccess['calculated']['requestedScenarios'][number] } | null>(null)
  const scenarioTrigger = useRef<HTMLElement | null>(null)
  const selectedScenario = scenarioSelection?.analysis.positionVersion === strategy.version && messages.some(message => message.analysis === scenarioSelection.analysis) ? scenarioSelection : null
  const scenarioAnalysis = [...messages].reverse().find(message => message.analysis?.positionVersion === strategy.version && message.analysis.calculated?.requestedScenarios?.length)?.analysis
  const previewScenario = (analysis: NonNullable<ChatMessage['analysis']>, result: SparringSuccess['calculated']['requestedScenarios'][number]) => {
    if (reviewRequest.current || analysis.positionVersion !== strategyRef.current.version) return
    scenarioTrigger.current = document.activeElement instanceof HTMLElement ? document.activeElement : null
    setScenarioSelection({ analysis, result })
  }
  ...
  const valuationView = view === 'table' || (!!analysisState?.legs.length && view === 'heatmap' && (!showAmericanPreview || strategy.valuationModel === 'american-crr-1024-v1')) ? view : undefined
  useEffect(() => {
  ...
  const commit = (next: StrategyState, remember = true, automaticUpdate = false) => {
    ...
    if (reviewRequest.current) {
      reviewGeneration.current++; reviewRequest.current?.abort(); reviewRequest.current = null; setPending(false)
      setMessages(items => [...items, { role: 'argus', text: 'The position changed during this review. Your question is kept above; ask again using the updated position.', note: 'STALE REVIEW · no changes applied' }])
    }
    setProposal(null)
    setEditError('')
    return true
  ...
  const loadMarket = async (preserve: boolean, dates?: string[], symbol = strategyRef.current.underlying, center = preserve && marketSnapshot ? marketSnapshot.strikeCenter ?? marketSnapshot.spot : undefined, capture = false, automaticUpdate = false) => {
    ...
    if (!/^[A-Z]{1,6}$/.test(symbol)) return setEditError('Enter a standard equity / ETF ticker or XSP using 1–6 letters.')
  ...
  const spar = async (prompt = composer, probabilityRange?: { lower: number; upper: number }, source?: { position: StrategyState; snapshot: MarketSnapshot }, firstExpiryRange?: { min: number; max: number }, discovery = false) => {
    ...
    try {
      ...
      if (candidateSelection) {
        ...
        if (!comparison.candidateComparison || JSON.stringify(data.calculated.positionComparison) !== JSON.stringify(comparison.candidateComparison)) throw new Error('Candidate discussion failed deterministic comparison validation. Your held position and selected candidate are unchanged.')
        const intent = parseComparisonIntent(data.calculated.comparisonIntent)
        if (!intent) throw new Error('Candidate discussion intent failed validation. Your held position and selected candidate are unchanged.')
        const rendered = renderCandidateComparison(comparison.candidateComparison, intent)
        if (data.reply.text !== rendered.text || JSON.stringify(data.reply.assumptions) !== JSON.stringify(rendered.assumptions) || JSON.stringify(data.reply.objections) !== JSON.stringify(rendered.objections) || JSON.stringify(data.reply.suggested_prompts) !== JSON.stringify(rendered.suggested_prompts)) throw new Error('Candidate discussion wording failed deterministic validation. Your held position and selected candidate are unchanged.')
      }
  ...
  return (
    <main className={`app-shell${compact ? ` integrated-workbench${optimizerScreen ? ' optimizer-screen' : ''}` : ''}`}>
      ...
      {compact && <nav className="workbench-screen-nav" aria-label="Workbench and optimizer"><button aria-current={!optimizerScreen ? 'page' : undefined} onClick={() => setOptimizerScreen(false)}>{optimizerScreen ? 'Back to Workbench' : 'Workbench'}</button><button aria-current={optimizerScreen ? 'page' : undefined} disabled={!analysisState || !marketSnapshot || !!marketSnapshot.historical || pending || chainPending || workspaceBusy || !!proposal} onClick={() => { stopAutomatic(); setOptimizerScreen(true) }}>Optimize</button><span>{optimizerScreen ? 'Compare alternatives · Inspect returns to Workbench · Apply is separate' : 'Your position · thesis, analysis and conversation'}</span></nav>}
      {compact && <section className="workbench-intent" aria-label="Trading thesis">
        ...
        <WorkbenchThesis key={strategy.underlying} state={analysisState} thesis={strategy.thesis} onChange={updateThesis} optimizeEnabled={!!marketSnapshot && !marketSnapshot.historical && !pending && !chainPending && !workspaceBusy && !proposal} onOptimize={target => { setOptimizerScreen(true); setOptimizerTarget(previous => ({ ...target, version: strategy.version, underlying: strategy.underlying, launch: (previous?.launch ?? 0) + 1 })) }} />
      </section>}

      {compact && optimizerScreen && <section className="optimizer-context"><h1>Optimize your trade</h1><p>{thesis || 'Compare ways to express your view. Your current position stays unchanged until Apply.'}</p><small>{strategy.underlying} · dated quote search · target and risk constraints below</small>{editError && <div className="calculation-error" role="alert"><strong>Edit not applied</strong><span>{editError}</span><button onClick={() => setEditError('')}>Dismiss</button></div>}</section>}
      ...
      <div className="workspace">
        <Library className="strategy-rail">
          ...
          <div className="template-list">
            {families.filter(family => visibleTemplates.some(item => item.family === family)).map((family) => <section key={family}><h2>{family}</h2>{visibleTemplates.filter((item) => item.family === family).map((item) => <button key={item.id} aria-pressed={activeId === item.id} className={activeId === item.id ? 'active' : ''} disabled={cashIndex && item.family === 'Stock + options'} title={cashIndex && item.family === 'Stock + options' ? 'Cash-index options cannot hold shares' : undefined} onClick={() => pickTemplate(item.id)}><i>{item.glyph}</i><span>{item.label}</span><kbd>{item.id.includes('calendar') ? '2x' : item.id.includes('iron') ? '4x' : ''}</kbd></button>)}</section>)}
          </div>
        ...
        <section className="builder">
          ...
          <div className="comparison-controls" role="group" aria-label="Saved position adjustment">
            <button className="undo-button" disabled={workspaceBusy || !savedIdentity} onClick={() => { if (!savedIdentity) return; stopAutomatic(); setLotSaved(savedIdentity.id) }}>Compare adjustment</button>
            <small>{savedIdentity ? `${savedIdentity.title} · saved inventory versus a hypothetical close, partial close or roll. Builder legs supply optional new openings; nothing is recorded until you confirm actual fills.` : 'Save or load a position to compare a close, partial close or roll.'}</small>
          </div>
          ...
          <div className="canvas-card" id="workspace-chart" tabIndex={-1} role="region" aria-label="Strategy chart">
            ...
            {editError && !(compact && optimizerScreen) && <div className="calculation-error" role="alert"><strong>Edit not applied</strong><span>{editError}</span><button onClick={() => setEditError('')}>Dismiss</button></div>}
            ...
            {scenarioAnalysis && <section className="scenario-selection" aria-label="Calculated what-if scenarios"><strong>Explore this review on the chart</strong><p>{scenarioAnalysis.positionUnderlying} · {scenarioAnalysis.positionName} · workspace v{scenarioAnalysis.positionVersion} · {scenarioAnalysis.positionSnapshot ? `${scenarioAnalysis.positionSnapshot.source} · retrieved ${scenarioAnalysis.positionSnapshot.retrievedAt} · ${scenarioAnalysis.position.pricing?.basis} basis` : "Sample/manual prices"}. Read-only: selection does not apply changes.</p><div>{scenarioAnalysis.calculated.requestedScenarios.map((result, index) => <button key={result.id} aria-pressed={selectedScenario?.result === result} onClick={() => previewScenario(scenarioAnalysis, result)}>What-if {index + 1} · ${result.scenario.scenarioSpot.toFixed(2)} · {result.scenario.scenarioDate.slice(0, 10)} · IV {signed(result.scenario.ivShift * 100, 2)} pts</button>)}</div></section>}
            {selectedScenario ? <ScenarioPreview key={`${selectedScenario.analysis.positionVersion}:${selectedScenario.result.id}`} baseline={selectedScenario.analysis.position} snapshot={selectedScenario.analysis.positionSnapshot} result={selectedScenario.result} onClose={closeScenario} /> : !analysisState ? <section aria-label="Empty analysis selection" role="status"><h3>No holdings included in analysis</h3><p>Add an option or shares, or include retained legs below. No P/L, Greeks or expiry probabilities are calculated for an empty selection.</p>{hasExclusions && <button onClick={() => commit({ ...strategyRef.current, excludedLegIds: undefined })}>Include all legs</button>}</section> : !displayBasis ? <p role="status">Risk percent unavailable: choose P/L or position value. This position has no positive exact expiry loss bound.</p> : view === 'curve' ? <PayoffChart range={chartRange} pnlDisplay={activePnlDisplay} metric={chartMetric} state={analysisState} breakevens={breakevenResult?.source === analysisState ? breakevenResult.value : undefined} comparison={comparisonCompatible ? comparedPosition : undefined} comparisonLabel={proposal ? 'Proposed' : comparingSaved ? `Saved revision ${savedIdentity!.revision}` : 'Frozen baseline'} onDragStart={stopAutomatic} onStrikeCommit={(source, next) => {
            ...
            }} /> : view === 'table' ? <ScenarioTable state={analysisState} pnlDisplay={activePnlDisplay} data={currentValuation?.table} error={valuationError} onSelect={updateScenario} /> : <div className="heatmap-wrap">{showAmericanPreview && !americanModel ? <AmericanHeatmap range={chartRange} state={analysisState} pnlDisplay={activePnlDisplay} /> : <Heatmap state={analysisState} pnlDisplay={activePnlDisplay} points={currentValuation?.heatmap} error={valuationError} onSelect={updateScenario} />}<div className="heatmap-scale"><span>{activePnlDisplay === 'position-value' ? 'ZERO VALUE' : 'LOSS'}</span><i style={activePnlDisplay === 'position-value' ? { background: 'linear-gradient(90deg, rgba(117,153,233,.08), rgba(117,153,233,.7))' } : undefined} /><span>{activePnlDisplay === 'position-value' ? 'LARGER ABSOLUTE VALUE' : 'PROFIT'}</span></div><p className="heatmap-help">{showAmericanPreview && !americanModel ? 'American comparison · 1,024-step lattice · continuous yield. Read-only; hover or use arrows to inspect. AI can inspect seven American checkpoints at the selected date. Headline metrics and probabilities remain European. No discrete dividends, assignment or exercise cashflows.' : 'Click a cell to apply its date and spot. Arrow keys inspect; Enter or Space applies. Values are modeled, not forecasts.'}</p></div>}
          ...
          <div className="conversation" ref={conversationPane}>
            ...
            {messages.map((message, index) => <article key={index} className={`message ${message.role}`}><header>{message.role === 'user' ? 'YOU' : message.role === 'guide' ? 'ARGUS GUIDE' : 'ARGUS'}<time>{index ? 'now' : 'workspace'}</time></header>
              ...
              {message.analysis && <div className="analysis-evidence">
                ...
                {!!message.analysis.calculated?.requestedScenarios?.length && <RequestedScenarios scenarios={message.analysis.calculated.requestedScenarios} baseline={message.analysis.position} onPreview={message.analysis.positionVersion === strategy.version ? result => previewScenario(message.analysis!, result) : undefined} />}
                ...
                {message.analysis.calculated?.scenario && <details className="source-evidence" aria-label="Captured analysis calculations"><summary>Calculations for this analysis</summary><small>{message.analysis.calculated.scenario.date} · spot {message.analysis.position.underlyingKind === 'cash-index' ? `${message.analysis.calculated.scenario.spot.toFixed(2)} index points` : `$${message.analysis.calculated.scenario.spot.toFixed(2)}`}</small>{message.analysis.calculated.scenario.valuation && <section><b>Position valuation · USD</b><span>Entry estimate (signed) {signed(message.analysis.calculated.scenario.valuation.signedEntryEstimate, 2)}</span><span>Cost allowance {signed(message.analysis.calculated.scenario.valuation.feeAllowance ?? 0, 2)}</span><span>Model value (signed) {signed(message.analysis.calculated.scenario.valuation.signedModelValue, 2)}</span><span>Option intrinsic (signed) {signed(message.analysis.calculated.scenario.valuation.signedIntrinsicValue, 2)}</span>{message.analysis.calculated.scenario.valuation.signedStockValue !== undefined && <span>Stock mark (signed) {signed(message.analysis.calculated.scenario.valuation.signedStockValue, 2)}</span>}<span>Model residual (signed) {signed(message.analysis.calculated.scenario.valuation.signedModelResidual, 2)}</span><span>Modeled P/L {signed(message.analysis.calculated.scenario.valuation.modelPnl, 2)}</span><p>Model value − entry estimate − cost allowance = modeled P/L. {message.analysis.calculated.scenario.valuation.signedStockValue !== undefined ? 'Model value − stock mark − option intrinsic = model residual' : 'Model value − intrinsic = model residual'}, not a share of entry cost. Long leg entry/model/intrinsic amounts add; short leg amounts subtract. Residual and P/L may have either sign. {message.analysis.calculated.scenario.valuation.entryBasis} {message.analysis.calculated.scenario.valuation.stockBasis}</p></section>}{message.analysis.calculated.scenario.legs.map(leg => <section key={leg.legId}><b>{leg.legId} · {leg.moneyness}</b><span>Model value {message.analysis!.position.underlyingKind === 'cash-index' ? `${leg.modelValuePerShare.toFixed(4)} premium points` : `$${leg.modelValuePerShare.toFixed(4)} / share`}</span><span>Intrinsic {message.analysis!.position.underlyingKind === 'cash-index' ? `${leg.intrinsicPerShare.toFixed(4)} premium points` : `$${leg.intrinsicPerShare.toFixed(4)} / share`}</span><span>Model value minus intrinsic {message.analysis!.position.underlyingKind === 'cash-index' ? `${leg.modelValueMinusIntrinsicPerShare.toFixed(4)} premium points` : `$${leg.modelValueMinusIntrinsicPerShare.toFixed(4)} / share`}</span></section>)}{message.analysis.calculated.scenario.timeStep && <section><b>Repriced through {shortDate(message.analysis.calculated.scenario.timeStep.date)}</b><span>Modeled value change {signed(message.analysis.calculated.scenario.timeStep.changeInValue, 2)} USD over {message.analysis.calculated.scenario.timeStep.calendarDays} calendar day(s)</span><p>{message.analysis.calculated.scenario.timeStep.assumptions}</p></section>}<p>{message.analysis.calculated.scenario.basis}</p></details>}
            ...
            {proposal && <p className="comparison-scenarios">Scenario: {proposal.comparisonBaseline ? 'held baseline at target' : 'current'} {(proposal.comparisonBaseline ?? strategy).underlyingKind === 'cash-index' ? `${(proposal.comparisonBaseline ?? strategy).scenarioSpot.toFixed(2)} index points` : `$${(proposal.comparisonBaseline ?? strategy).scenarioSpot.toFixed(2)}`} on {shortDate((proposal.comparisonBaseline ?? strategy).scenarioDate)}, IV {signed(strategy.ivShift * 100, 1)} pts; proposed {proposal.next_state.underlyingKind === 'cash-index' ? `${proposal.next_state.scenarioSpot.toFixed(2)} index points` : `$${proposal.next_state.scenarioSpot.toFixed(2)}`} on {shortDate(proposal.next_state.scenarioDate)}, IV {signed(proposal.next_state.ivShift * 100, 1)} pts.</p>}
            {proposal?.comparisonBaseline && <section aria-label="Optimizer target comparison"><h3>Held versus candidate at search target</h3><p>Held entries and costs are unchanged. Both positions are modeled at {proposal.comparisonBaseline.underlyingKind === 'cash-index' ? `${proposal.comparisonBaseline.scenarioSpot.toFixed(2)} index points` : `$${proposal.comparisonBaseline.scenarioSpot.toFixed(2)}`} on {proposal.comparisonBaseline.scenarioDate}, using the captured valuation time and model. This read-only comparison does not change the workspace scenario. Different expiry payoffs are not being equated; the curves show target-date modeled P/L, not realized returns.</p><PayoffChart state={proposal.comparisonBaseline} comparison={comparedPosition} comparisonLabel="Candidate at target" metric="pnl" readOnly /></section>}
            ...
            {proposal?.lossBound && <p aria-label="Candidate conservative first-expiry bound">Conservative first-expiry loss bound: {money(proposal.lossBound.amount)} at {proposal.lossBound.date}. Exact maximum loss and profit are not established. {proposal.lossBound.basis} This calculation does not establish pre-expiry or lifetime loss caps. Assignment, funding and slippage are excluded. Target P/L divided by this bound is a scenario ratio, not a return forecast.</p>}
