...
function ScenarioPreview({ baseline, snapshot, result, onClose }: { baseline: StrategyState; snapshot?: MarketSnapshot; result: SparringSuccess['calculated']['requestedScenarios'][number]; onClose: () => void }) {
  ...
  const preview = useMemo(() => {
    try {
      ...
      if (legIvShifts !== undefined && (!Array.isArray(legIvShifts) || legIvShifts.length > 4)) throw Error()
...
function LegRow({ leg, snapshot, fixedEntry, onChange, onRemove }: { leg: OptionLeg; snapshot?: MarketSnapshot; fixedEntry?: boolean; onChange: (patch: Partial<OptionLeg>) => void; onRemove: () => void }) {
  ...
  return (
    <div className="leg-row">
      <span className={`leg-index ${leg.side}`}>{leg.side === 'long' ? 'B' : 'S'}</span>
...
type PendingProposal = SparringSuccess & { before: SparringSuccess['metrics'] }
...
export function App() {
  ...
  const quoteMark = useMemo(() => marketSnapshot ? quoteValuation(strategy, marketSnapshot) : null, [strategy, marketSnapshot])
  ...
  const displayBasis = pnlDisplayBasis(strategy, activePnlDisplay)
  ...
  const valuationView = view === 'table' || (strategy.legs.length > 0 && view === 'heatmap' && (!americanPreview || strategy.valuationModel === 'american-crr-1024-v1')) ? view : undefined
  useEffect(() => {
    const controller = new AbortController()
    requestWorkspaceValuation(strategy, controller.signal, valuationView, chartRange).then(value => {
    ...
    return () => controller.abort()
  }, [strategy, valuationAttempt, valuationView, chartRange])
  ...
  const commit = (next: StrategyState, remember = true, automaticUpdate = false) => {
    ...
    const errors = validateStrategy(candidate)
    if (errors.length) {
    ...
    if (!candidate.legs.length) { setView('curve'); setAmericanPreview(false) }
    setProposal(null)
    setEditError('')
    return true
  }
  ...
  const loadMarket = async (preserve: boolean, dates?: string[], symbol = strategyRef.current.underlying, center = preserve && marketSnapshot ? marketSnapshot.strikeCenter ?? marketSnapshot.spot : undefined, capture = false, automaticUpdate = false) => {
    ...
    try {
      ...
      const errors = validateMarketStrategy(next, snapshot)
      if (errors.length) throw new Error(errors.join('; '))
  ...
  const addQuotedLeg = (contractId: string, side: OptionLeg['side']) => {
    ...
    const errors = validateMarketStrategy(next, marketSnapshot)
    if (errors.length) return setEditError(errors.join('. '))
    commit(next)
  }

  const addLeg = () => {
    if (strategy.legs.length >= 4) return
  ...
  const analyzeRemaining = (state: StrategyState, snapshotId: string) => {
    ...
    if (workspaceBusy || snapshotId !== strategyRef.current.pricing?.snapshotId || !snapshot || state.pricing?.snapshotId !== snapshotId || state.pricing.entryMode !== 'fixed' || state.feeAllowance !== 0 || validateStrategy(state).length || validateMarketStrategy(state, snapshot).length) return false
  ...
  const inspectCandidate = async (analysis: NonNullable<ChatMessage['analysis']>, id: string) => {
    if (reviewRequest.current) return
    const search = analysis.calculated.candidateSearch, snapshot = analysis.positionSnapshot, current = strategyRef.current
    const candidate = search?.candidates.find(item => item.id === id)
    if (!search || !candidate || !snapshot || search.baseVersion !== current.version || analysis.positionVersion !== current.version || search.snapshotId !== current.pricing?.snapshotId || search.model !== (current.valuationModel ?? 'european-bsm-v1')) return setEditError('This candidate search is stale. Search again using the current position and quotes.')
    stopAutomatic()
    const controller = new AbortController()
    reviewRequest.current = controller
  ...
  const mixedExpiry = strategy.legs.some((leg) => leg.expiry !== strategy.legs[0].expiry)
  const americanModel = strategy.valuationModel === 'american-crr-1024-v1'
  const stockOnly = strategy.legs.length === 0
  const firstExpiry = stockOnly ? Date.parse(strategy.scenarioDate) : Math.min(...strategy.legs.map(leg => Date.parse(leg.expiry)))
  ...
  const comparisonCompatible = comparedPosition && strategy.underlying === comparedPosition.underlying && strategy.valuationTimestamp === comparedPosition.valuationTimestamp && (strategy.valuationModel ?? 'european-bsm-v1') === (comparedPosition.valuationModel ?? 'european-bsm-v1') && Date.parse(strategy.scenarioDate) === Date.parse(comparedPosition.scenarioDate) && (!strategy.legs.length || !comparedPosition.legs.length || firstExpiry === Math.min(...comparedPosition.legs.map((leg) => Date.parse(leg.expiry))))
  ...
  const briefQuestion = stockOnly ? 'Compare share quantities or add an option to model a hedge.' : strategy.legs.every(leg => Date.parse(leg.expiry) <= Date.parse(strategy.scenarioDate))
      ...
      <div className="workspace">
        ...
        <section className="builder">
          <div className="context-strip">
            ...
            <div className="workspace-actions"><button className="undo-button" disabled={!marketSnapshot || pending || chainPending || workspaceBusy} onClick={() => { stopAutomatic(); setHistoryOpen(true) }}>Price history</button><button className="icon-button" aria-label="Reset strategy" onClick={() => pickTemplate((activeId ?? 'iron-condor') as TemplateId)}>↻</button><button className="undo-button" aria-label="Undo" disabled={!history.length} onClick={undo}>↶ Undo</button></div>
          </div>
          ...
          {historyOpen && <PriceHistory state={strategy} onClose={() => setHistoryOpen(false)} />}
          ...
          {marketSnapshot && <div className="market-pricing-panel">
            ...
            {quoteMark && <details className="quote-model-comparison" aria-label="Quote and model comparison">
              ...
              <p>Effective IV: {strategy.legs.map(leg => `${leg.side} ${leg.contracts} × $${leg.strike} ${leg.type} (${shortDate(leg.expiry)}): ${(effectiveIv(strategy, leg) * 100).toFixed(2)}%`).join('; ')}.</p>
          ...
          <div className="metric-ribbon">
            ...
            </> : <p role="status">{valuationError ? <>{valuationError} <button onClick={() => { setValuationResult(undefined); setValuationAttempt(value => value + 1) }}>Retry calculation</button></> : 'Calculating current position…'}</p>}
          </div>
          ...
          {mixedExpiry && metrics && <details className="source-evidence"><summary>Refine first-expiry high and low</summary><FirstExpiryAnalysis state={strategy} min={metrics.sampledRange.spotMin} max={metrics.sampledRange.spotMax} busy={pending || (view === 'heatmap' && americanPreview && !americanModel)} onAsk={() => void spar(`Explain the first-expiry minimum and maximum P/L intervals between spot ${metrics.sampledRange.spotMin} and ${metrics.sampledRange.spotMax}, remaining search uncertainty and separate tail and assignment risks. Do not change the position.`, undefined, undefined, { min: metrics.sampledRange.spotMin, max: metrics.sampledRange.spotMax })} /></details>}
          {mixedExpiry && metrics && <details className="source-evidence"><summary>Find first-expiry breakeven candidates</summary><FirstExpiryBreakevens state={strategy} min={metrics.sampledRange.spotMin} max={metrics.sampledRange.spotMax} result={breakevenResult} onResult={setBreakevenResult} /></details>}

          <div className="canvas-card" id="workspace-chart" tabIndex={-1} role="region" aria-label="Strategy chart">
            <div className="canvas-head"><div><span className="eyebrow">Position landscape</span><h2>{view === 'table' ? 'Scenario table' : view === 'curve' ? chartMetric === 'pnl' ? displayBasis?.label ?? 'Unavailable' : `${chartMetrics[chartMetric].label} exposure` : 'Price × time surface'}</h2></div><div className="chart-controls">{view === 'heatmap' && !americanModel && <button className="undo-button" aria-pressed={americanPreview} onClick={() => setAmericanPreview(value => !value)}>{americanPreview ? 'Return to European' : 'Preview American'}</button>}{view === 'curve' && <select aria-label="Chart metric" value={chartMetric} onChange={event => setChartMetric(event.target.value as ChartMetric)}>{Object.entries(chartMetrics).map(([key, value]) => <option key={key} value={key}>{value.label}</option>)}</select>}<div className="segmented"><button className={view === 'curve' ? 'active' : ''} onClick={() => setView('curve')}>Curve</button><button className={view === 'heatmap' ? 'active' : ''} disabled={stockOnly} onClick={() => setView('heatmap')}>Heatmap</button><button className={view === 'table' ? 'active' : ''} onClick={() => setView('table')}>Table</button></div></div><div className="scenario"><label>IV shift · pts <ScenarioInput label="IV shift in percentage points" value={Number((strategy.ivShift * 100).toFixed(2))} onCommit={(value) => updateScenario({ ivShift: value / 100 })} /></label><label>At spot · $ <ScenarioInput label="Scenario spot" value={strategy.scenarioSpot} onCommit={(value) => updateScenario({ scenarioSpot: value })} /></label></div></div>
            ...
            {!stockOnly && <div className="scenario-time">
            ...
            <div className="chart-controls comparison-controls"><button className="undo-button" disabled={!!proposal} onClick={() => { setManualBaseline(structuredClone(strategy)); setView('curve') }}>{manualBaseline ? 'Replace baseline' : 'Freeze comparison'}</button>{manualBaseline && <button className="undo-button" onClick={() => setManualBaseline(undefined)}>Clear baseline</button>}</div>
            ...
            {!displayBasis ? <p role="status">Risk percent unavailable: choose P/L or position value. This position has no positive exact expiry loss bound.</p> : view === 'curve' ? <PayoffChart range={chartRange} pnlDisplay={activePnlDisplay} metric={chartMetric} state={strategy} breakevens={breakevenResult?.source === strategy ? breakevenResult.value : undefined} comparison={comparisonCompatible ? comparedPosition : undefined} comparisonLabel={proposal ? 'Proposed' : 'Frozen baseline'} onDragStart={stopAutomatic} onStrikeCommit={(source, next) => {
              if (strategyRef.current === source && next !== source) commit(next)
            }} onStrike={(source, id, strike, group, step) => {
              if (strategyRef.current !== source) return source
        ...
        <aside className="sparring-rail">
              ...
              {message.analysis && <div className="analysis-evidence">
                ...
                {message.analysis.calculated?.candidateSearch && <section className="verified-risk" aria-label="Ranked quoted candidates">
                  ...
                  {message.analysis.calculated.candidateSearch.candidates.map((candidate, rank) => <div key={candidate.id}>
                    ...
                    <button className="preview-chart-button" disabled={pending || chainPending || workspaceBusy || message.analysis!.positionVersion !== strategy.version || message.analysis!.calculated.candidateSearch!.snapshotId !== strategy.pricing?.snapshotId} onClick={() => void inspectCandidate(message.analysis!, candidate.id)}>Inspect candidate</button>
                  </div>)}
          ...
          <div className="prompt-chips"><button onClick={() => spar('What breaks this trade?')}>Break the thesis</button><button onClick={() => spar('Compare this with a calendar spread.')}>Compare structure</button><button onClick={() => spar('Reduce downside without adding a fifth leg.')}>Reduce downside</button></div>
