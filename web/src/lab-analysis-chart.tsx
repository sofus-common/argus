...
export function LabAnalysisChart({ state, snapshot, thesisTarget, onChange, onPositionChange, onAsk }: { state: StrategyState; snapshot?: MarketSnapshot; thesisTarget?: number; onChange: (state: StrategyState) => void; onPositionChange?: (state: StrategyState) => void; onAsk?: () => void }) {
  ...
  const timeSteps = Math.ceil((expiry - valuation) / 3_600_000)
  ...
  return <section className="lac" aria-label="Interactive trade analysis">
    <header className="canvas-head"><h2>{view === 'curve' ? chartMetrics[metric].label : view === 'table' ? 'Scenario table' : 'Price × time'}</h2><div className="chart-controls"><select aria-label="Chart metric" value={metric} disabled={view !== 'curve'} onChange={event => setMetric(event.target.value as ChartMetric)}>{Object.entries(chartMetrics).map(([key, value]) => <option key={key} value={key}>{value.label}</option>)}</select><div className="segmented" aria-label="Analysis views">{(['curve', 'heatmap', 'table'] as const).map(name => <button key={name} className={view === name ? 'active' : ''} aria-pressed={view === name} onClick={() => setView(name)}>{name[0].toUpperCase() + name.slice(1)}</button>)}</div></div><div className="scenario"><label>Scenario spot · $<ScenarioInput label="Scenario spot" value={state.scenarioSpot} onCommit={value => select({ scenarioSpot: value })}/></label></div></header>
    <div className="scenario-time"><label><span className="scenario-date-heading">Scenario date · UTC<button onClick={() => select({ scenarioDate: state.valuationTimestamp })}>Now</button></span><input type="datetime-local" aria-label="Scenario date UTC" step="0.001" min={new Date(valuation).toISOString().slice(0, -1)} max={new Date(expiry).toISOString().slice(0, -1)} value={new Date(state.scenarioDate).toISOString().slice(0, -1)} onChange={event => { const at = Date.parse(`${event.target.value}Z`); if (Number.isFinite(at)) select({ scenarioDate: new Date(at).toISOString() }) }}/></label><div><input type="range" aria-label="Scenario time" aria-valuetext={`${new Date(state.scenarioDate).toISOString().slice(0, 16).replace('T', ' ')} UTC`} min={0} max={timeSteps} step={1} value={Date.parse(state.scenarioDate) === expiry ? timeSteps : (Date.parse(state.scenarioDate) - valuation) / 3_600_000} onChange={event => select({ scenarioDate: new Date(Math.min(expiry, valuation + Number(event.target.value) * 3_600_000)).toISOString() })}/><small><span>Valuation · {state.valuationTimestamp.slice(0, 10)}</span><span>First expiry · {new Date(expiry).toISOString().slice(0, 10)}</span></small></div></div>
    ...
    <ChartRangeControls spot={state.spot} range={range} onChange={setRange}/></details><div className="chart-inspection"><button onClick={onAsk} disabled={!onAsk}>Ask about this chart</button></div>
  </section>
}
