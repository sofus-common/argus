...
export function LabStrategyPicker({ onSelect, onClose }: { onSelect: (id: TemplateId) => void; onClose: () => void }) {
  ...
  return <dialog ref={dialog} className="lsp-dialog" aria-labelledby={titleId} onCancel={event => { event.preventDefault(); onClose() }}>
    ...
    <p className="lsp-disclosure">{TEMPLATES.length} structures · {supported.length} interactive sample templates. Other structures are available in the full app only.</p>
