...
3. **The experiment.** Diagram: snapshot → 24 candidates → quant picks / AI
...
6. **Governance in one slide.** 16 gates, hash-bound authorization, kill
...
7. **Evidence.** Hash-chained ledger, claim manifest with statuses, credential-
   free replay, 13 tests. "AI beats quant: UNSUPPORTED" shown deliberately.
...
"Every thirty minutes ARGUS freezes an Alpaca snapshot of SPY and QQQ option
chains and builds every five-dollar-wide debit vertical near forty delta —
about twenty-four candidates, each with its max loss, quoted spread, realized-
versus-implied vol, term slope and trend. A deterministic quant picks one by a
...
"Before any order, sixteen checks re-observe quotes and the account: one
percent per position, five percent aggregate, no stacking the same spread,
